...
Success is `00`. Every request starts with `pageNo=1&numOfRows=999`. Since #2590 a full first page whose `totalCount` is larger is followed by sequential requests for pages 2..n (only `pageNo` changes; at most 5 pages). Each continuation page must pass the same envelope checks, repeat `totalCount`, hold exactly the remaining rows up to 999, differ from earlier pages and, when the body echoes `pageNo`/`numOfRows`, match the request. The merged rows must equal `totalCount` before any organizer runs. A first page shorter than 999 with a different `totalCount`, or any failing page, fails the grid without publishing or storing a prefix. The short forecast (`getVilageFcst`, issue-reported `totalCount` 1,016) therefore costs two requests per grid; single-page products are unchanged. No retry-policy change is introduced. Day-3–10 sea wave fields each read their own source field; distinct-value fixtures cover every field, including nonfinite values in days 4–7.
...
| [controllerTown24h](../../server/controllers/controllerTown24h.js), `adjustShort` | Existing code splits positive r06/s06 across pairs of adjacent records based on PTY, starting at index 2. Characterization proves 1.5/0.5 become rounded 0.8/0.3 in both adjacent entries. This is an exposed legacy assumption, **not** an approved conversion for hourly input. It can misrepresent hourly amounts and rounding can change totals. |
| [controllerTown](../../server/controllers/controllerTown.js), RSS merge, `adjustShort`, current interpolation and string conversion | RSS can fill sentinels; temperature feeds summaries/interpolation; rain/snow strings use legacy fields. Existing time-slot and quantity assumptions remain. |
| [controllerTown24h](../../server/controllers/controllerTown24h.js), `_convertWeatherData` | Snow cm-to-mm and requested-unit conversion are unit conversion, not period aggregation. |

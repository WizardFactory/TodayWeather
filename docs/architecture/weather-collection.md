...
1. Parse requested coordinates into Mongo order `[longitude, latitude]` and query stored records sorted by `dateObj`.
2. Select yesterday, today and current records using local-time calculations. The current freshness window is 15 minutes. A yesterday record counts only when it was fetched after that local day ended (`pubDate`); the previous day's last `today` refresh still holds forecasts for its later hours, so the first request of a day makes one `combined` call for the observed day. Hours missing from an accepted record (for example on a daylight-saving change) no longer trigger a refetch.
3. If all three exist, no provider call is made. Otherwise `_requestDatas()` takes a per-location lock in `vc.fetch.locks` (`_id` `"<lon>,<lat>"`, 10-second `expireAt` TTL; an expired lock is taken over; the holder deletes only its own lock, matched by `expireAt`; if the lock store errors, the worker fetches anyway), re-reads the records, and makes one Timeline call: `combined` (`yesterday/next7days`) when yesterday is missing, otherwise `forecast` (`today/next7days`). A worker that does not get the lock re-reads every 250 ms for up to 2.5 seconds and returns once current and today exist. After a failed call the lock is kept for 2 more seconds, so a failing location makes at most one provider attempt per 2 seconds, and the gateway's retry after its 3-second timeout can fetch again.
4. `vcConverter` turns the Timeline body into Dark Sky-format yesterday/today/current documents in `us` units, so `_parseData()` and the world merge/unit code are unchanged. The zone comes from Visual Crossing `timezone`. The UTC offset is the one in effect at request time, taken from the latest hour row (local `datetime` against `datetimeEpoch`), because the top-level `tzoffset` is the offset at the start of the requested range and differs after a daylight-saving change. Google time-zone lookups are no longer made on this path. The `address.country` field stores that IANA zone string, not an ISO country code. `_saveData()` upserts by `geo` plus `dateObj`, current last; save errors are logged while the response continues with fetched data.
5. If the provider call fails but stored current and today exist, they are returned without yesterday. Otherwise the error reaches the existing `next(err)`/500 path.
6. The world-weather controller converts local times and merges daily/current/hourly records, sets `source: "VC"` and `pubDate.VC`; route middleware converts requested units and computes descriptions and summaries.

`vcRequester` uses a shared keep-alive HTTPS agent, a 2.5-second budget for the whole call and one retry only on HTTP 429 within 1.5 seconds, inside that budget. It logs `VC> range=... status=... cost=<queryCost> ms=...` and never logs the key; a missing or placeholder `VC_SECRET_KEY` fails without a network call. Measured on 2026-09-26, a `combined` call cost 25 records and a `forecast` call 1 record. The legacy `dsfRequester` no longer contains a Dark Sky URL and returns a "Dark Sky API retired" error immediately. `maintainDB()` runs every day when started by the gather manager and removes records older than two days. It is not started automatically in `service` mode.

Sources: [DSF route](../../server/routes/v000902/route.dsf.coord.v000902.js), [world controller](../../server/controllers/worldWeather/controllerWorldWeather.js), [DSF cache controller](../../server/controllers/worldWeather/dsf.controller.js#L675-L853), [Visual Crossing requester](../../server/lib/VC/vcRequester.js), [converter](../../server/lib/VC/vcConverter.js), [lock model](../../server/models/worldWeather/vc.fetch.lock.model.js), [record model](../../server/models/worldWeather/dsf.model.js), [world cache sequence](../rewrite/diagrams/server-world-cache-sequence.html).

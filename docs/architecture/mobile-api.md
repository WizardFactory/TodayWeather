...
## Fresh AWS minute current fields (#2573)

The v000901–903 KMA middleware chains add `enrichCurrentByAwsMinute` after history/forecast composition and before `insertIndex` and unit conversion. When `AWS_MINUTE_ENRICH_ENABLED=true`, it independently reads the new UTC minute store; legacy hourly/city absence does not block this read. Normal composition stays intact. Fresh, suitable station observations replace `t1h/reh/vec/wsd` independently only when newer than a proven source time; unknown-provenance valid values remain unchanged. Invalid/missing fields may be filled. No new snapshot enters `currentList`, hourly history or forecast arrays.

Additive `minuteObservation.fields` preserves each changed field's UTC source time and station. `liveTime` advances only for a new temperature source and pairs with additive `liveDate` (KST); old `date/time` remain for existing comparisons. Day/night icons use `liveDate` where present. Rain/cloud/weather fields retain their ordinary sources. Disabled/missing/stale/failed lookups preserve fallback. Enriched origin responses have at most 120 seconds of cache age, additionally capped by remaining 20-minute eligibility; the external Lambda's recorded 300-second header and app's ten-minute memory gate remain distinct operational checks. See [operator/cache contract](../operations/aws-minute.md) and [diagram](diagrams/aws-minute.html).

...
| Overseas (DSF-named) cache | Current data window 15 minutes; reads limited to the last 3 days; stored yesterday used without hourly completeness checks; per-location Mongo lock; on failure a stored current record up to 3 hours old is served | A cache miss makes one Visual Crossing call (`combined` without yesterday, else `forecast`). The response waits at most 2.5 seconds, while the lock holder's call may run for 8 seconds and still store its records; lock waiters poll up to 2.5 seconds. A failed fetch blocks the location for 2 seconds; a daily-limit 429 or rejected key stops calls for 10 minutes |

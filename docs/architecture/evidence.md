...
| How does the overseas (DSF-named) cache work? | `dsf.controller.js`: `_findDataFromDB`, `_checkProvider`, `_requestDatas`, `_acquireLock`, `_waitForRecords`, `_fetchVc`, `_fallback`, `_recordUsage`, `_saveData`, `maintainDB`; [Visual Crossing requester](../../server/lib/VC/vcRequester.js), [converter](../../server/lib/VC/vcConverter.js); [DSF model](../../server/models/worldWeather/dsf.model.js), [fetch-lock model](../../server/models/worldWeather/vc.fetch.lock.model.js), [usage model](../../server/models/worldWeather/vc.usage.model.js) |
...
3. **Provider viability is unverified.** KMA, WU and other URLs are historical source contracts. No claim about today's service support or migration target is made for them. Dark Sky was retired in March 2023; since #2585 the overseas path uses Visual Crossing, and the legacy Dark Sky requester fails immediately. The deployed gateway Lambda can still call Dark Sky reverse geocoding (#2601).

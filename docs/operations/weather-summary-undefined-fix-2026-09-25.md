...
- `server/controllers/controllerKmaStnWeather.js` `updateWeather`: treats `weatherType < 0` like `undefined`, so unmapped text falls back to sky (no precipitation) or pty (rain/sleet/snow).
...
- `server/test/offline/weather-desc.test.js` (registered in `npm run test:offline`) covers four cases: the wording map, empty labels, the updateWeather fallback, and both summary builders. It needs no DB or network.
...
- KMA may emit other strings not yet mapped; `Fail weatherStr=` in `www-error-*.log` is the signal. With the follow-up revision, `updateWeather` falls back to sky/pty text. On the currently deployed first revision, such a string leaves `weatherType: -1` and `weather: ""`. `summaryWeather` omits the weather item, but `current.summary` (`makeSummary`, unguarded there) can still end in `", "`.

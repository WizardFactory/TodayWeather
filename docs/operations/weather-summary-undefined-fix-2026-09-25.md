...
- `server/test/offline/weather-desc.test.js` (in `npm run test:offline` and the rss-offline Node 16/22 matrix) covers the wording map, empty labels, the updateWeather sky/PTY 1–7 fallback and both summary builders. `weather-desc-response-smoke.js` runs the actual v000903 route for 14 scenarios: observed wording; `-1` with the sky, pty 1 and nowcast pty 5 fallbacks; no station text with pty 5 (the #2573 `hourlyMissing` path); and an invalid sky giving `weather ""`. It asserts no swallowed exceptions. Neither needs a DB or network.
...
## Interaction with #2573/#2574 (2026-09-25)

#2574 lets `getStnHourlyAndMinRns` continue with minute data when hourly rows are missing (`hourlyMissing`). That path has no station text, so it relies on the `updateWeather` sky/PTY fallback from this fix. The real 2026-09-25 19:00 city page in #2574 (`server/test/offline/fixtures/city-obs-2026-09-25.html`, 97 stations) contains only `약한비연속적`, `약한비단속적`, `보통비단속적`, `비끝`, `흐림` and blank cells. All of them map with this fix. See the [station observation runbook](kma-station-observations.md).

...
| Daily/combined weather | Valid future daily rows, forecast-only KMA rain without an assumed 24-hour period, AM/PM icons, min/max range, overview | Confirm every legacy ancillary metric/provider format |
...
- `packages/weather-core/`: source-aware, unit-aware normalization without mutating provider payloads. KMA `-50` temperatures and negative nonnegative metrics become unavailable; valid zero remains zero. Old daily rows are excluded by source date. KMA daily rain uses forecast `r06`, never accumulated observation `rn1`; missing forecasts remain unavailable. Zero snowfall stays in data but is hidden in the UI, while positive values below 0.1 display as `<0.1` in the selected unit.
- `web/`: React/TypeScript UI, versioned local preferences, IndexedDB snapshots keyed by location and all units, query cancellation and explicit stale/demo notices. Normalization cache-key revision v2 separates new weather query/snapshot keys from older daily values; favorites and settings are preserved. Coordinates are rounded to three decimals; sharing uses a curated public city ID rather than exact current location.

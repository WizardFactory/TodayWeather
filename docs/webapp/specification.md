...
Status: proposed; 2026-09-24. Authority and acceptance: [intent](intent.md). Architecture: [technical design](technical-design.md). Delivery: [implementation plan](implementation-plan.md).

## Product promise and audience

Users should be able to open a link on mobile or desktop and understand today's weather relative to yesterday, upcoming hourly/daily changes and air quality. Returning users can manage the same categories of preferences and city notifications as the mobile products. Installation and location permission are optional for browsing; supported installed browsers can receive notifications.

Primary journeys: checking before leaving home, comparing saved cities, inspecting a pollutant/station, reviewing national conditions/warnings, and receiving scheduled/condition-based city notifications. Preserve weather meaning and actions while adapting density and interaction to the browser.
...
`Required` means required for a supported-browser parity release. `Adapted` preserves the outcome with web interaction. `Decision` is unresolved and must be accepted or implemented before claiming that area is equivalent. Internal delivery stages do not remove requirements.
...
| S07 Units | `/settings/units`: all six dimensions and the existing supported value sets | Required: request/display/cache/notification units agree |
...
| S09 Notifications | `/notifications/:locationId`: alert window, alarm weekdays/times, save/cancel, installation/permission state | Adapted but required: server persistence, actual supported-device delivery, unsubscribe, timezones and duplicates |
...
All rows derive from [S01–S16 definitions](../rewrite/screen-specifications.md); exact routes above are **new proposals**.
...
| iOS/Android widgets and Watch | Keep existing native apps; offer app link and web dashboard | Native-only: not claimed as equivalent PWA widgets/Watch app |
| Daily/condition push | Web subscription plus server scheduler/condition evaluation | OS/network controls delivery; current-location alarms use a clearly named last saved location until the app updates it |
...
4. **Notifications:** choose city → explain capability → install guidance where needed → user grants permission → subscribe → configure weekday/time and alert window → save → server acknowledgement → optional test notification. Cancel leaves previous settings. Failure/denial never shows a saved/enabled success state.
...
6. **Offline:** load a previously visited shell and snapshot → persistent offline notice and observed/fetched time → cached forecast remains inspectable; no fresh-data claim. First-ever offline visit explains unavailable data. Disable notification-setting saves until the server can confirm them.
7. **Update:** new app version detected → offer reload when no unsaved notification form is present → activate compatible app/cache schema. Never discard a form automatically or imply that refreshing the app refreshed provider observations.
...
- Supported-browser baseline: release-time current and previous major Safari/iOS Safari, Chrome/Android Chrome, Edge and Firefox for weather browsing; push/install capabilities are separately detected and tested. Pin exact versions in release evidence, not this timeless requirement.
...
- Fresh-data success, failed searches, source age, API latency/errors, permission conversion and push persistence/provider/display outcomes are measured separately, without precise coordinates or subscription keys in analytics.

## Scope decisions

AK has confirmed the parity priority, not the framework, monetization, domain or schedule. Recommended defaults and tradeoffs are in the [technical design](technical-design.md). Paid entitlement, login/sync and separate TodayAir branding remain decisions, not hidden requirements. The [implementation plan](implementation-plan.md) sequences them without blocking independent UI/adapter work.

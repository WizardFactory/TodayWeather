...
Locations are normalized to three decimal places by `geolocationNormalize`. Compare this with native/provider precision requirements before adopting it as a universal rule. The URL branch and push helper both treat latitude zero as absent. Direct world (`/dsf/coord`) responses use `location.lon`; the public gateway historically overwrites this with client-compatible `location.long`. Preserve that adapter explicitly when replacing or bypassing the gateway; the converter itself does not rename the field.

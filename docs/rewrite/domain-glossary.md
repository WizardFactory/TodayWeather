...
Since 5e653285 the domestic collector calls `VilageFcstInfoService_2.0` on `apis.data.go.kr`: `getUltraSrtNcst` for the current observation, `getUltraSrtFcst` for the shortest (very-short) forecast and `getVilageFcst` for the short forecast ([collector URLs](../../server/lib/collectTownForecast.js#L144-L152)). At `ff7acf39` it called the retired `SecndSrtpdFrcstInfoService2` (`ForecastGrib`, `ForecastTimeData`, `ForecastSpaceData`); the unit and code comments below come from that era. Each XML `category` is matched explicitly and stored with `parseFloat`, with no scaling ([short](../../server/lib/collectTownForecast.js#L618-L743), [shortest](../../server/lib/collectTownForecast.js#L745-L856), [current](../../server/lib/collectTownForecast.js#L858-L972)). The exceptions are the new-service categories `PCP`, `SNO`, `TMP` and shortest `RN1`, which go through [`parseMeasurement`](../../server/lib/collectTownForecast.js#L545-L562): a complete decimal (optional `mm`/`cm` suffix) is kept, `강수없음`/`적설없음` become `0`, and range or threshold text such as `1~4` or `1.0mm 미만`, blanks and negative amounts become the field sentinel. A "0.1" in a source comment is provider resolution, not a stored scale factor. The period caveats of the new categories are in [gather source reconciliation](../architecture/gather-source-reconciliation.md#period-contract-and-consumer-limitations).
...
| --- | --- | --- |
| Collector completeness check | Rejects a batch when its first record still has the template value for key fields (short: `sky`, `reh`, `pty`, `t3h`; shortest: `sky`, `reh`, `pty`, `t1h`; current: `rn1`, `pty`, `t1h`) or when its temperature is below `-100`. Short and shortest also reject negative `pty`, `sky` or `reh`. Current deletes other negative `pty`, `rn1` and `reh` values of the first record instead. Since 5e653285 [`_emitOrganizedData`](../../server/lib/collectTownForecast.js#L565-L588) then checks **every** record: an empty batch, any non-finite number or empty `wf*` text rejects it, and for short/shortest each record must hold `sky`, `reh`, `pty` ≥ 0 and a temperature that is neither `-50` nor below `-100`. The response envelope is validated first (`resultCode` `00` and a positive `totalCount` since 5e653285; `totalCount` equal to the item count since b4d063fd, counted over all pages since #2590, [L475](../../server/lib/collectTownForecast.js#L475)) | [short](../../server/lib/collectTownForecast.js#L709-L737), [shortest](../../server/lib/collectTownForecast.js#L822-L850), [current](../../server/lib/collectTownForecast.js#L924-L965), [envelope](../../server/lib/collectTownForecast.js#L355-L477) |
...
| Medium sea (`seaString`) | `wf3Am` … `wf10`, `wh3AAm` … `wh10B` | Wave heights; model default `-100`. b4d063fd fixed days 4–7, which at `ff7acf39` all copied the day-3 `wh3*` values ([collector L1115-L1130](../../server/lib/collectTownForecast.js#L1115-L1130)) | Not read by `getMid` (only forecast, land and temperature are read) |
...
- Since #2585 the "DSF" inputs are Visual Crossing values converted to Dark Sky format by [`vcConverter`](../../server/lib/VC/vcConverter.js#L27-L83): the icon is the Visual Crossing icon, replaced by the precipitation type when an observed row has precipitation, and `pre_type` is `rain`, `snow` or `sleet` (mixed snow or freezing rain/ice). The rules below apply unchanged.
- DSF `_getPrecType(icon, pre_pro, pre_type)` ([L2647-L2677](../../server/controllers/worldWeather/controllerWorldWeather.js#L2647-L2677)): the provider icon `rain` → 1, `snow` → 2, `sleet`/`hail` → 3. Any other icon, or no icon, with probability `< 0.5` gives 0. Otherwise, and also after an icon match, `pre_type` `rain`/`snow`/`sleet` sets 1/2/3 and overrides the icon result (for example icon `rain` with `pre_type` `snow` gives 2).
- DSF hourly 3-hour bucket ([L2766-L2779](../../server/controllers/worldWeather/controllerWorldWeather.js#L2766-L2779)) evaluates three hourly inputs: any 1 gives 1; any 2 adds 2 (rain + snow = 3); any 3 gives 3.
- The WU mergers used by the `/ww` routes add 1 for rain and 2 for snow ([L2274-L2280](../../server/controllers/worldWeather/controllerWorldWeather.js#L2274-L2280), [L2346-L2352](../../server/controllers/worldWeather/controllerWorldWeather.js#L2346-L2352)).
...
| Part | Values | KMA rule ([`_parseSkyStateLowCase`](../../server/controllers/controllerTown24h.js#L1984-L2035)) | World rule ([`_parseWorldSkyState`](../../server/controllers/worldWeather/controllerWorldWeather.js#L2457-L2513)) |
| --- | --- | --- | --- |
...
Invalid `sky` (for example `-1`) or `pty` logs an error and contributes no part. For example, `sky -1, pty 0` by day yields `sun`. World cloud cover comes from DSF `cloud × 100`; since #2585 the daily value clamps the `-100` sentinel (a provider 0) to 0, as does daily wind speed ([cloud](../../server/controllers/worldWeather/controllerWorldWeather.js#L2561-L2564), [wind](../../server/controllers/worldWeather/controllerWorldWeather.js#L2589-L2593) in `_makeDailyDataFromDSF`). The hourly value averages the positive values of three hourly inputs and defaults to `0` ([L2725-L2740](../../server/controllers/worldWeather/controllerWorldWeather.js#L2725-L2740)). The DSF merges also leave temperatures and feels-like temperatures unset when the stored value is the `-100` sentinel ([`_isValue`](../../server/controllers/worldWeather/controllerWorldWeather.js#L2516-L2524)), and set wind direction, pressure and daily humidity only when positive, so the sentinel is not converted into a response value; hourly and current humidity turn it into 0.
...
| World current (`thisTime`) | `_isNight(curDate, daily.data)` takes the first provider daily record whose date prefix is ≤ the current time. **Night iff `current < sunrise` or `current ≥ sunset`** | [L2605-L2619](../../server/controllers/worldWeather/controllerWorldWeather.js#L2605-L2619) |
| World hourly | `_getSunTime` matches the converted daily row by the first 10 date characters. **Day iff `sunrise < t ≤ sunset`**, strict at sunrise. With no matching day, both values are `undefined` and the row is night | [L2630-L2638](../../server/controllers/worldWeather/controllerWorldWeather.js#L2630-L2638), [L2715-L2723](../../server/controllers/worldWeather/controllerWorldWeather.js#L2715-L2723) |
| World daily | Always day | [L2601](../../server/controllers/worldWeather/controllerWorldWeather.js#L2601) |
...
| `rn1Str`, `r06Str`, `s06Str` | Since #2583: formatted from the parsed amount or category bounds, rain (`r06`, `rn1`) in mm and snow (`s06`) in cm. Exact → `"<v>mm"` (`10mm`); below a threshold → `"~<max>mm"` (`~1mm`); range → `"<min>~<max>mm"` (`30~51mm` for a slot sum); open-ended → `"<min>~?mm"` (`50~?mm`). Present for a positive amount; a zero amount only when `pty` matches (rain 1, 2, 4, 5, 6; snow 2, 3, 6, 7; `rn1` any `pty > 0`). The retired 1/5/10/20/40/70/100 table is gone | Literal `mm`/`cm` text, independent of the requested unit | [kmaPrecipitation.js](../../server/lib/kmaPrecipitation.js), [`_makeStrForKma`](../../server/controllers/controllerTown.js), [mobile API](../architecture/mobile-api.md#forecast-precipitation-amounts-issue-2583) |
...
- `s06Str` used to look up buckets after `s06` was multiplied by 10; since #2583 it is formatted from the snow total in cm kept before conversion.
...
[`WeatherDescription`](../../server/controllers/controller.weather.desc.js) maps English (DSF-format summary; since #2585 generated by `vcConverter` from a fixed vocabulary such as `light rain`, `partly cloudy`, `fog`) and Korean (KMA station) phrases to `weatherType` 0–66. `getWeatherStr` maps each code to one `LOC_*` key; the array has 67 entries (evaluated with Node). A phrase is lower-cased and cut at the first `" and "`. Unknown phrases give `-1` and log an error. KMA current receives `weatherType` from station observations ([controllerTown.js](../../server/controllers/controllerTown.js#L1901-L1911), [station adjustments](../../server/controllers/controllerKmaStnWeather.js#L20-L75)). World rows derive it from the DSF summary text ([hourly L2820-L2821](../../server/controllers/worldWeather/controllerWorldWeather.js#L2820-L2821), [current L2849-L2850](../../server/controllers/worldWeather/controllerWorldWeather.js#L2849-L2850)).
...
| World `mergeAqi` (DSF chain) | Each WAQI per-pollutant index → `extractValue`, then `so2`/`no2`/`o3` ppb → ppm (`ppb2ppm`). `airnow`: `<p>Grade` = `index2Grade` of the provider index, `aqiValue` = provider `aqi`, `aqiGrade` = maximum pollutant grade. `airkorea_who`: `value2grade`, `aqiValue` = maximum index, `aqiGrade` = maximum grade. `airkorea`: `value2grade`, `aqiValue` = maximum index **+ 75** when three or more pollutant indices exceed 100, **+ 50** when two do, and `aqiGrade` = `index2Grade`. Other values (`aqicn`): `value2index('aqicn')` and `index2Grade('aqicn')` | [L2008-L2092](../../server/controllers/worldWeather/controllerWorldWeather.js#L2008-L2092) |
...
3. **World `aqicn` label (source reading, not reproduced).** The `aqicn` branch calls `UnitConverter.airGrade2Str(thisTime.aqiGrade, 'aqi', res)` with the arguments in the wrong order ([L2091](../../server/controllers/worldWeather/controllerWorldWeather.js#L2091)). The grade number is passed as `airUnit` and `'aqi'` as the grade, so `aqiStr` becomes `""` and a warning is logged.
...
- **Provider semantics.** Unit, code and averaging-period meanings come from source comments. The KMA, AirKorea, DSF/DarkSky, Visual Crossing and WAQI specifications were not consulted, except that the Visual Crossing units follow `unitGroup=us`. The retired KMA service may encode values differently from its successors.

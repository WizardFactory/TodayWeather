...
Invalid `sky` (for example `-1`) or `pty` logs an error and contributes no part. For example, `sky -1, pty 0` by day yields `sun`. World cloud cover comes from DSF `cloud × 100`; since #2585 the daily value clamps the `-100` sentinel (a provider 0) to 0, as does daily wind speed ([cloud](../../server/controllers/worldWeather/controllerWorldWeather.js#L2546-L2549), [wind](../../server/controllers/worldWeather/controllerWorldWeather.js#L2574-L2578) in `_makeDailyDataFromDSF`). The hourly value averages the positive values of three hourly inputs and defaults to `0` ([L2710-L2725](../../server/controllers/worldWeather/controllerWorldWeather.js#L2710-L2725)).

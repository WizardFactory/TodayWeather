...
| 65 | `GET /v000903/kma/addr/:region` | [#L62](../../server/routes/v000903/route.kma.v000903.js#L62) | 37-item reference list ([appendix](server-response-assembly.md#kma-v000903)) | **App:** stored address without coordinates ([service.weatherutil.js#L129-L140](../../client/www/js/service.weatherutil.js#L129-L140)). **Gateway:** `/weather/v000903/coord` with a KR result (deployment observation). **Web PWA:** indirectly, through the same gateway path with canonical units ([direct-api.ts#L187-L222](../../web/src/direct-api.ts#L187-L222)). **Internal:** `/v000903/nation` fan-out | `/v000903/kma/addr/{location}` 1,320 in total (all 200), covering rows 65–67. It equals 88 origin-reaching nation requests × 15 ([§6](server-response-assembly.md#6-nationwide-screens-regional-air-plus-http-fan-out)) |
...
The provider is Visual Crossing since #2585; the routes keep their `dsf` names. Request-time provider calls, cache writes and the world envelope (`source:'VC'`, `pubDate.VC`) are described in [world weather](server-response-assembly.md#5-world-weather-request-time-cache-fill-then-merge).

| # | Route | Source | Chain | Side effects | Consumers | 30-day traffic |
| ---: | --- | --- | --- | --- | --- | --- |
| 70 | `GET /v000901/dsf/coord/:loc` | [route.dsf.coord.js#L33-L37](../../server/routes/v000901/route.dsf.coord.js#L33-L37) | 13 items, with no `skyIconLowCase` (CamelCase icons) | One Visual Crossing Timeline call and WAQI provider calls on a cache miss, plus DB writes, a short `vc.fetch.locks` entry and a daily `vc.usage` counter update | **Gateway:** default version for unversioned non-KR `/weather/coord` (deployment observation). **Internal (inference):** the out-of-Korea 302 from the KMA chains, followed by an unknown client | 0 (in scope) |
...
| 3–7 `queryTwoDaysWeatherNewForm`, `convertDsfLocalTime`, `mergeDsfDailyData`, `mergeDsfCurrentDataNewForm`, `mergeDsfHourlyData` | Same | Same request-time Visual Crossing/WAQI cache fill and merge |
| 8 `mergeAqi` | Same | Without `airUnit`, the AQI grade uses the `aqicn` index branch ([controllerWorldWeather.js#L2025-L2092](../../server/controllers/worldWeather/controllerWorldWeather.js#L2025-L2092)) |
| 9 `dataSort` | Same | — |

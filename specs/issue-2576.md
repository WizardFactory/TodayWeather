...
- **R3 (amended):** in `updateWeather`'s pty ≥ 1 `case 0–12` branch, when pty is 4 the text is `소나기`/25 (r3 correction: the code sets `소나기`, which is the type-25 label), 5 `약한비`/19, 6 `약진눈깨비`/29, 7 `약한눈`/33, in addition to the existing 1/2/3 → 65/64/66. This applies to every type 0–12 in this branch, including `-1`/`undefined` after they are set to 0. Unknown pty (for example 8) keeps the existing behaviour. With pty 0 and a sky outside 0–4, the type stays `-1` and `getWeatherStr` gives `""`.
...
## Amendment r3 (2026-09-25): base refresh to #2574
- **R1 (regression):** `보통비단속적`, observed in the real 2026-09-25 city page fixture, maps to 20.
- **R3 (path):** when #2574 marks `hourlyMissing` there is no station `weather`, so `weatherType` stays undefined and the same sky/PTY 1–7 fallback applies.
- **R5:** the route smoke adds "no station text + nowcast pty 5 → 19". The fixture mapping check is recorded as investigation evidence rather than a CI test, because it needs cheerio.
- **R6:** `mobile-api.md` links the #2573 section. It states the precise smoke coverage (pty 1 and 5 at route level; 4, 6 and 7 in the unit test), that pty ≥ 1 outside 1–7 keeps type 0, and that drizzle/showers drop the `계속`/`단속` suffix. The operations note links the enrichment-restore record now on master.

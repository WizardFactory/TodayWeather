/**
 * Created by Peter on 2017. 3. 22..
 */

"use strict";

var req = require('request');
var fcRequester = require('../../lib/FC/fcRequester');
var assert  = require('assert');
var config = require('../../config/config');
var Logger = require('../../lib/log');
var convertGeocode = require('../../utils/convertGeocode');
var keybox = require('../../config/config').keyString;
global.log  = new Logger(__dirname + "/debug.log");

describe('unit test - FC', function(){
    var list = [
        /*
         {
         address: '221B Baker Street London NW1 England',
         lat: 51.52,
         lon: -0.15
         },
         {
         address: 'Place du Palais 84000 Avignon France',
         lat: 43.95,
         lon: 4.80
         },
         {
         address: 'Piazza del colosseo 1 00184 Rom Italy',
         lat: 41.89,
         lon: 12.49
         },
         {
         address: 'Parthenon 10558 Athens Greece',
         lat: 37.97,
         lon: 23.72
         },
         {
         address: 'Akihabara Station 17-6',
         lat: 35.70,
         lon: 139.77

         },
         {
         address: 'Fukuoka Station 321',
         lat: 33.57,
         lon: 130.42
         },
         */
        {
            address: 'Kita6Jonishi 4-Chome',
            lat: 43.06,
            lon: 141.34
        },
        {
            address: 'NY',
            lat: 40.67,
            lon: -73.94
        }
    ];

    it('callect weather by FC', function(done){
        var fc = new fcRequester();
        fc.collect(list, keybox.fc_keys[0].key, function(err, result){
            if(err){
                log.error('!!! failed to get weather data');
                log.error(err);
                done();
                return;
            }

            log.info('!!! Successed to get weather data from Foreca');
            log.info(result);
            done();

        });
    });

    it('get weather by FC', function(done){
        var fc = new fcRequester();
        fc.getForecast({lat:40.67, lon:-73.94}, keybox.fc_keys[0].key, function(err, result){
            if(err){
                log.error('!!! failed to get weather data');
                log.error(err);
                done();
                return;
            }

            log.info('!!! Successed to get weather data');
            log.info(result);
            done();

        });
    });

    it('test error case : ECONNRESET', function(done){
        var fc = new fcRequester();
        var count = 1;

        fc.get = function(url, option, callback){
            if(count-- > 0) {
                return callback({code:"ECONNRESET"});
            }else{
                return req.get(url, option, callback);
            }
        };

        fc.getForecast({lat:40.67, lon:-73.94}, keybox.fc_keys[0].key, function(err, result){
            if(err){
                log.error('!!! failed to get weather data');
                log.error(err);
                done();
                return;
            }

            log.info('!!! Successed to get weather data');
            log.info(result);
            done();

        });
    });

    it('test error case : ETIMEDOUT', function(done){
        var fc = new fcRequester();
        var count = 1;

        fc.get = function(url, option, callback){
            if(count-- > 0) {
                return callback({code:"ETIMEDOUT"});
            }else{
                return req.get(url, option, callback);
            }
        };

        fc.getForecast({lat:40.67, lon:-73.94}, keybox.fc_keys[0].key, function(err, result){
            if(err){
                log.error('!!! failed to get weather data');
                log.error(err);
                done();
                return;
            }

            log.info('!!! Successed to get weather data');
            log.info(result);
            done();

        });
    });
});

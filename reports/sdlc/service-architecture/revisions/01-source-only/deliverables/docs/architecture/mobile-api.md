...
## Public API versus implemented backend

| Boundary | Implemented evidence | What remains unknown |
| --- | --- | --- |
| Public `/weather/v000903/...` | Client constructs it | Gateway mount, region/provider dispatch, response enrichment and cache policy |
| Public `/geocode/v000903/...` | Client constructs it | Implementation and routing |
| Backend `/v000903/kma/addr/...` | Mounted Express router with KMA pipeline | Which public paths route to it in deployment |
| Backend `/v000903/kma/coord/:loc` | Resolves address via `API_SERVER/geocode/coord/:loc`, then runs KMA pipeline | Availability and country/address behavior of external geocoder |
| Backend `/v000903/dsf/coord/:loc` | Reuses v000902 DSF middleware pipeline | Public routing into this path |
| Backend `/v000903/geo/:loc` | Calls external geocoder; redirects KR to KMA address, other countries to DSF | Whether public gateway uses this legacy redirect endpoint |

`route.geo.v000903` constructs relative redirects without forwarding the original query string explicitly. It is different from `ControllerTown24h.coord2addr`, which continues the same request after filling KMA address parameters. The existence of `/geo` does not prove the public `/weather` mount rewrites to it.

The diagrams show the public boundary as external and its mapping as unverified. A standard Express start alone cannot satisfy all client-built paths. Sources: [mounts](../../server/app.js), [v000903 router](../../server/routes/v000903/index.js), [geo redirect](../../server/routes/v000903/route.geo.v000903.js), [KMA coordinate middleware](../../server/controllers/controllerTown24h.js).
...
| HTTP cache | No universal weather TTL identified; KMA warnings explicitly use 300 seconds | Do not treat client memory TTL as a CDN/API cache rule |

...
The file-backed reminder store uses atomic rename and restrictive permissions, with one serial writer. **Never scale it to multiple app processes/replicas.** The 30-second scheduler runs in this process and provides generic city reminders, not condition evaluation or precise-time guarantees. Dedupe is persisted before send: a crash can miss a reminder rather than duplicate it. There is no catch-up/retry queue for missed schedules. Web Push 404/410 clears the invalid subscription. Inactive installations expire after 90 days.

...
## Service EC2 continuation
1. Use the explicitly authorized local SSH key against the named service host. Read process, socket, selected config, deployment and startup metadata; retain only sanitized evidence.
2. Compare deployed commit and selected file hashes with local source. Update EC2 internals, source/runtime distinctions and canonical guidance.
3. Add the EC2 internal diagram and update AWS infrastructure, service overview and mobile request diagrams. Retain collection/KAQ source-boundary diagrams with unchanged artifact hashes.
4. Validate six diagram bindings, local links, shared guidance, source preservation and selective ignore rules. Independently verify exact candidate and remote evidence consistency.
5. Preserve HTML and JSON, ignore visual-check PNGs and the private key. No remote changes, application probes, lateral SSH, commit or push. Rollback only locally authored documents and diagrams; preserve user-provided access guidance and ignore rules.
